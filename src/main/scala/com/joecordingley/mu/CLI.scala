...
object CLI extends AvailabilityDefinedInteractions[IO] {
  ...
  def availableCardsMap(s: Set[Card]): SortedMap[Int, Card] =
    SortedMap(s.toList.zipWithIndex.map(_.swap): _*)

  def availableCardsString(playerString: String,
                           availableCards: SortedMap[Int, Card]) =
    s"player $playerString, choose from these cards\n" +
      availableCards
        .map {
          case (k, Card(suit, rank)) => s"$k: $suit ${rank.value}"
        }
        .mkString("\n")
  def parseUserString(s: String): Option[Set[Int]] =
    s.split(",").toList.traverse(toInt _).map(_.toSet)
...
trait AvailabilityDefinedInteractions[F[_]] {
  ...
  def askPlayerForPartner(player: Player,
                          availablePartners: Set[Player]): F[Player]
